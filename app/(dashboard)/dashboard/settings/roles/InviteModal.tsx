...
export default function InviteModal({ canInvite }: { canInvite: boolean }) {
  ...
  const [result, setResult] = useState<{ success: boolean; error?: string; emailSent?: boolean; inviteUrl?: string | null } | null>(null)
            ...
            {result?.success ? (
              <div className="px-6 py-10 text-center space-y-3">
                {result.emailSent ? (
                  ...
                  <>
                    <div className="w-10 h-10 mx-auto rounded-full bg-yellow-500/10 border border-yellow-500/20 flex items-center justify-center">
                      <svg className="w-5 h-5 text-yellow-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}>
                        <path strokeLinecap="round" strokeLinejoin="round" d="M12 9v4m0 4h.01M10.29 3.86 1.82 18a2 2 0 0 0 1.71 3h16.94a2 2 0 0 0 1.71-3L13.71 3.86a2 2 0 0 0-3.42 0Z" />
                      </svg>
                    </div>
                    <p className="text-white font-bold text-sm">Invite created, but the email didn&apos;t send</p>
                    <p className="text-white/40 text-xs">Copy this link and share it with them directly (expires in 24 hours):</p>
                    {result.inviteUrl && (
                      <button
                        onClick={() => navigator.clipboard?.writeText(result.inviteUrl!)}
                        className="w-full mt-1 px-3 py-2 rounded-lg bg-white/[0.06] border border-white/10 text-[11px] text-white/60 font-mono truncate hover:bg-white/10 transition-colors"
                        title="Click to copy"
                      >
                        {result.inviteUrl}
                      </button>
